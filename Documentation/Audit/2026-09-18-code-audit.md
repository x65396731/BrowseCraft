...
**真机复核未通过，本节结论按实测下调**：装上修改后的包再看 178 列表，同一张封面的 `stage=image`
仍是 **2 到 3 行**（第 1 页 `/upload/book/8.jpg` 等各三次，第 2 页同形）。因此**零尺寸那一次确实存在、
也确实被消掉了，但它不是重复的主因**。日志进一步显示重复是**按网格行成组出现**的
（三列一组、整组重复 2 到 3 次），说明 `.task(id:)` 因标识变化而重跑了多次，
而标识里唯一会变的就是 `displaySize`——即布局尺寸被报了多个非零取值。
真因待下一轮带尺寸的instrumentation 测量后再定，**不得再凭推断下结论**。

（2026-09-18 记：本节一度写成「真机复核通过」，那是把爱壹帆列表那份日志——9 个可见单元各一行——
误当成 178 的复核结果。同一份改动在两个站上的表现不同，这一点本身也需要解释。）
