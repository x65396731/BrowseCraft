...
public enum VideoRuleBranchState: Sendable {
    case empty
    case nonEmpty
}

public struct VideoRuleAPIBranch<Value: Sendable>: Sendable {
    public init(
        value: Value,
        state: VideoRuleBranchState,
        ...
        self.extractionLog = extractionLog
        self.finalURL = finalURL
    }

    public let value: Value
    public let state: VideoRuleBranchState
    public let requestLog: SourceRequestLog
    public let extractionLog: SourceExtractionLog
    public let finalURL: URL
}

// 中文注释：API loader 只负责请求与传输；list/detail/episode API 响应解释
// 全部由 BrowseCraftCore 执行。
public struct VideoSourceAPILoader: Sendable {
