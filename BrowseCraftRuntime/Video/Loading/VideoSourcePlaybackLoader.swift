...
public struct VideoSourcePlaybackLoader: Sendable {
    ...
    public func executeWithRouteFacts(
        _ session: VideoPreparedPlaybackExecutionSession
...
public enum VideoHLSInitialManifestClassification: Hashable, Sendable {
    case unknown
    case encrypted
}

public struct VideoHLSInitialManifestInspection: Hashable, Sendable {
    public init(
        classification: VideoHLSInitialManifestClassification,
        observedFinalURL: URL?
    ) {
    ...
    public let classification: VideoHLSInitialManifestClassification
    public let observedFinalURL: URL?
}

// 中文注释：这里只做有界、纯文本 tag 分类；永不请求 key/IV，也永不把未命中升级为 unencrypted。
public enum VideoHLSManifestEncryptionClassifier: Sendable {
    public static let maximumAnalyzedByteCount: Int = 256 * 1_024

    public static func classify(_ manifest: String) -> VideoHLSInitialManifestClassification {
    ...
    public static func isPlayerUnplayableProtection(_ line: String, tag: String) -> Bool {
...
public struct VideoPlaybackNoiseAdmission: Sendable {
    public init(
        playback: VideoRuleParsedPlayback,
        discardedMediaCount: Int,
        discardedIframeCount: Int,
        extractionLogs: [SourceExtractionLog],
        issues: [SourceRuntimeIssue]
    ) {
        self.playback = playback
        self.discardedMediaCount = discardedMediaCount
        self.discardedIframeCount = discardedIframeCount
        self.extractionLogs = extractionLogs
        self.issues = issues
    }

    public let playback: VideoRuleParsedPlayback
    public let discardedMediaCount: Int
    public let discardedIframeCount: Int
    public let extractionLogs: [SourceExtractionLog]
    public let issues: [SourceRuntimeIssue]

    public var absentMediaReason: VideoPreparedPlaybackRouteReason {
        return Self.reason(discarded: self.discardedMediaCount)
    }

    public var absentIframeReason: VideoPreparedPlaybackRouteReason {
        return Self.reason(discarded: self.discardedIframeCount)
    }

    /// 中文注释：iframe-resolve 途中「既没有 media 也没有 iframe」时用这一个，
    /// 因为两类候选都可能是被过滤掉的。
    public var absentCandidateReason: VideoPreparedPlaybackRouteReason {
