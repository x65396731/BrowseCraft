...
## 5.7 封面请求重复构造（2026-09-18）：两轮诊断被真机测量连续推翻

真机日志显示 178 列表页上同一张封面的 `stage=image` 出现 2 到 3 行。我为此提出并实施了一个修法，
又两次误报结论，最后靠一条打进请求构造入口的临时日志才把事实问清楚。过程本身比结论更该记。

**第一次诊断（错）**：认为 `displaySize` 初值为 zero，`.task(id:)` 先带零尺寸触发一次、
测量后再触发一次，而零尺寸那次不声明缩略选项、等于按原图解码。据此把
`RemoteImageRequestIdentity` 改成可失败构造（尺寸为零不构造、不发请求）。

**第一次误报（错）**：把爱壹帆列表那份日志（9 个可见单元各一行）当成 178 的复核结果，写了「复核通过」。
实测 178 仍是 2 到 3 行。已更正。

**第二次误报（错）**：加了临时定位日志却一行没出现，据此以为封面请求走的是别的代码路径。
事后用 `strings` 核对安装包，发现**那次的instrumentation根本没编进二进制**——那份日志什么也证明不了。
此后凡临时instrumentation，先验二进制再交付测量。

**真正的测量**（把三条日志写进源码、用户自己在 Xcode 里构建运行）：
...
| 每张封面的标识构造 | 4 次 |
| 每张封面真正进入请求构造 | 2 次 |
| 每次的布局尺寸 | 全部是 `112.333 × 168.935`，**逐位相同** |

三条结论随之确定：

1. **零尺寸那一次根本不发生**（零条 reject）。第一次诊断的前提不成立，可失败构造在这条路径上从不触发，
   它是防御性代码而不是本问题的修法。
2. **不是布局抖动**。四次构造的尺寸逐位相同。
3. **是视图身份被重建**。标识没变而 `.task(id:)` 仍重跑，只可能是视图被移除后重新插入。
   日志上游对得上：同一份 49 条数据被发布了两次（`SourcesViewModel` 的 `catalog-source-add`
   与 `LibraryViewModel` 的 `current-snapshot`），网格因此重建一次。

**代价比我此前说的小**：重复的只是请求构造（扫 Cookie 存储、合并三层请求头、写日志），
下载与解码不重复（Nuke 对相同请求做合并）。

**没有采用的修法**：按标识缓存已构造的 `ImageRequest`。请求在构造时把 Cookie 烤进了请求头，
而真机日志里 `hasCookie` 会随会话从 false 变 true；缓存会让请求带上过期的 Cookie，
而这个站恰好对图片请求的头很敏感（见 7.7 的 Referer 三次 403）。风险大于收益，放弃。

**判定：不动，依据是代价测量。** 动手修「重复发布」之前先量了一次请求构造值多少
（固定输入，2000 次取均值，模拟器）：
...
不到 3 毫秒，而且是每次进列表一次、不是每帧。为它加缓存要冒 Cookie 过期的风险（见上一段），
改发布时序要动两个 ViewModel，两者的收益都被这个读数否掉。**这一项到此关闭。**

代码层面查清的时序供后来人参考：`SourcesViewModel.addCatalogSource` 取到列表后把快照发进
`sourceSelectionStore`，`LibraryViewModel.applyPreparedSnapshotIfAvailable` 再取出应用——
那是一次生产加一次消费，不是两次加载。真正让网格重建两次的是这条流程后面的 tab 切换
（`RootView` 监听 `latestSourceAddID` 后切到 Library），**这一点尚未单独证实**，
要证实需要在真机上做 Instruments 采样，属另一类任务。

**这一节的教训比结论重要**：同一个现象我给出过三种诊断（零尺寸、走别的路径、布局抖动），
前两种被测量推翻、第三种被读数排除，而真正决定要不要修的是「值多少」这一个数字，
它本该是第一个去量的。

**顺带修掉一条我自己写的脆弱断言**（`WKWebViewDOMStabilityMeasurementTests`，F2-7 那轮引入）：
`testRepeatedOuterHTMLSerialisationCost` 断言「整页序列化不快于空转 JS」，本次全量跑时翻车
（小页面 1ms 对 2ms）。两者都由 JS 桥往返的固定开销主导，毫秒取整之后谁大谁小是噪声；
而 `Duration.milliseconds` 的注释本就写明**「只用于日志与测量输出，不参与判断」**，那条断言违反了它自己的约定。
改为直接比较两个夹具的 `Duration`：「1500 行的文档整页序列化必须比小页面贵」——
JS 桥的固定开销在两边相同会被抵消，剩下的差异只来自文档大小。连跑三轮稳定通过，
其中一轮正好复现了旧断言翻车的那种取值（小页面 1ms 对 2ms），新断言不受影响；
大页面 3 到 5ms 对小页面 1ms，余量充足。
