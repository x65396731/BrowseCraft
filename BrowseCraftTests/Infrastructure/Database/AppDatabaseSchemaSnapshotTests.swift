...
struct AppDatabaseSchemaSnapshotTests {
    ...
    // （SQLite 的 ALTER TABLE ADD COLUMN 把新列插在列定义末尾、表约束之前）与 v3.local-books（三张表、三个索引）、v4.book-progress-detached-from-local-books（进度与书签两张表重建、去掉对 local_books 的外键）、v5.book-reading-history（站点书阅读历史一张表、一个索引）、v6.remove-rss（删 rss_reading_history 一表两索引）。
...
table|book_reading_history|CREATE TABLE "book_reading_history" ("userID" TEXT NOT NULL REFERENCES "users"("id") ON DELETE CASCADE, "sourceID" TEXT NOT NULL, "detailURL" TEXT NOT NULL, "bookItemID" TEXT NOT NULL, "bookTitle" TEXT NOT NULL, "coverURL" TEXT, "chapterTitle" TEXT, "chapterURL" TEXT, "visitedAt" DATETIME NOT NULL, "sourceSnapshotJSON" TEXT, PRIMARY KEY ("userID", "sourceID", "detailURL"))
...
table|local_books|CREATE TABLE "local_books" ("id" TEXT PRIMARY KEY, "userID" TEXT NOT NULL REFERENCES "users"("id") ON DELETE CASCADE, "title" TEXT NOT NULL, "author" TEXT, "format" TEXT NOT NULL, "fileRelativePath" TEXT NOT NULL, "coverRelativePath" TEXT, "fileSHA256" TEXT NOT NULL, "byteCount" INTEGER NOT NULL, "importedAt" DATETIME NOT NULL, "lastOpenedAt" DATETIME)
table|sources|CREATE TABLE "sources" ("userID" TEXT NOT NULL REFERENCES "users"("id") ON DELETE CASCADE, "id" TEXT NOT NULL, "name" TEXT NOT NULL, "baseURL" TEXT NOT NULL, "type" TEXT NOT NULL, "kind" TEXT NOT NULL, "configJSON" TEXT NOT NULL, "enabled" BOOLEAN NOT NULL DEFAULT 1, "createdAt" DATETIME NOT NULL, "updatedAt" DATETIME NOT NULL, "deletedAt" DATETIME, "origin" TEXT, PRIMARY KEY ("userID", "id"))
...
index|idx_book_bookmarks_book_created_at|CREATE INDEX idx_book_bookmarks_book_created_at ON book_bookmarks(bookID, createdAt DESC)
index|idx_book_reading_history_user_visited_at|CREATE INDEX idx_book_reading_history_user_visited_at ON book_reading_history(userID, visitedAt DESC)
...
index|idx_local_books_user_imported_at|CREATE INDEX idx_local_books_user_imported_at ON local_books(userID, importedAt DESC)
index|idx_local_books_user_sha256|CREATE INDEX idx_local_books_user_sha256 ON local_books(userID, fileSHA256)
