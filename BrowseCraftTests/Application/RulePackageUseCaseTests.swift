...
struct RulePackageUseCaseTests {
    @Test func codecEncodesAndDecodesRulePackage() throws {
        let rule: SiteRule = try Self.completeV2Rule()
        let codec: RulePackageCodec = RulePackageCodec(
            now: {
                return Date(timeIntervalSince1970: 1_234)
            }
        )

        let packageJSON: String = try codec.encodePackage(
            rule: rule,
            sourceID: "user-rule",
            sourceName: "Exported Source",
            appVersion: "1.0-test"
        )
        let package: BrowseCraftRulePackage = try codec.decodePackage(packageJSON: packageJSON)

        #expect(package.formatVersion == RulePackageCodec.currentFormatVersion)
        #expect(package.kind == RulePackageCodec.packageKind)
        #expect(package.metadata.sourceID == "user-rule")
        #expect(package.metadata.sourceName == "Exported Source")
        #expect(package.metadata.sourceBaseURL == "https://example.test")
        #expect(package.metadata.ruleName == "Complete V2 Site")
        #expect(package.metadata.appVersion == "1.0-test")
        #expect(package.rule == rule)
        #expect(package.checksum == (try codec.checksum(for: rule)))
    }

    @Test func codecRejectsChecksumMismatch() throws {
        let rule: SiteRule = try Self.completeV2Rule()
        let codec: RulePackageCodec = RulePackageCodec(
            now: {
                return Date(timeIntervalSince1970: 1_234)
        ...
        let packageJSON: String = try codec.encodePackage(rule: rule)
        let tamperedPackageJSON: String = packageJSON.replacingOccurrences(
            of: "\"name\" : \"Complete V2 Site\"",
            with: "\"name\" : \"Tampered V2 Site\""
        )

        do {
            _ = try codec.decodePackage(packageJSON: tamperedPackageJSON)
            Issue.record("Expected checksum mismatch to fail.")
        } catch RulePackageError.checksumMismatch {
            #expect(true)
        } catch {
    ...
    @Test func codecRejectsUnsupportedPackageKind() throws {
        let rule: SiteRule = try Self.completeV2Rule()
        let codec: RulePackageCodec = RulePackageCodec(
            now: {
                return Date(timeIntervalSince1970: 1_234)
        ...
        let packageJSON: String = try codec.encodePackage(rule: rule)
        let invalidKindPackageJSON: String = packageJSON.replacingOccurrences(
            of: "\"kind\" : \"browsecraft.rule.package\"",
            with: "\"kind\" : \"other.rule.package\""
        )

        do {
            _ = try codec.decodePackage(packageJSON: invalidKindPackageJSON)
            Issue.record("Expected unsupported package kind to fail.")
        } catch RulePackageError.unsupportedKind(let kind) {
            #expect(kind == "other.rule.package")
        } catch {
            Issue.record("Unexpected error: \(error.localizedDescription)")
        }
    }

    @Test func codecRejectsUnsupportedFormatVersion() throws {
        let rule: SiteRule = try Self.completeV2Rule()
        let codec: RulePackageCodec = RulePackageCodec(
            now: {
                return Date(timeIntervalSince1970: 1_234)
        ...
        let packageJSON: String = try codec.encodePackage(rule: rule)
        let unsupportedVersionPackageJSON: String = packageJSON.replacingOccurrences(
            of: "\"formatVersion\" : 1",
            with: "\"formatVersion\" : 99"
        )

        do {
            _ = try codec.decodePackage(packageJSON: unsupportedVersionPackageJSON)
            Issue.record("Expected unsupported format version to fail.")
        } catch RulePackageError.unsupportedFormatVersion(let version) {
            #expect(version == 99)
        } catch {
            Issue.record("Unexpected error: \(error.localizedDescription)")
        }
    ...
    @Test func exportUseCaseBuildsPackageFromLatestSource() throws {
        ...
        let package: BrowseCraftRulePackage = try codec.decodePackage(packageJSON: export.packageJSON)
