...
struct ComicRuleAPIResponseEvaluator {
    static func evaluate(
        ...
        case .envelope:
            ...
            guard let rawStatus = ComicRuleAPIResolver.firstJSONValue(at: statusPath, in: json),
    ...
    private static func firstMatchedFailurePath(_ paths: [String], object: Any) -> String? {
        for path in paths {
            ...
            let values = ComicRuleAPIResolver.jsonValues(at: normalizedPath, in: object)
    ...
    private static func declaredMessage(in object: Any, paths: [String]?) -> String? {
        for path in paths ?? [] {
            ...
            let messages = ComicRuleAPIResolver.jsonValues(at: normalizedPath, in: object)
                .compactMap { value in
                    return ComicRuleAPIResolver.stringValue(value)?
...
private struct ComicRuleLegacyAPIResponseEvaluator {
    static func evaluate(json: Any) -> ComicRuleAPIResponseEvaluation {
        ...
        if let code = ComicRuleAPIResolver.stringValue(dictionary["code"]),
           code != "0" {
            let message = ComicRuleAPIResolver.stringValue(dictionary["message"])
            var details: [String] = []
    ...
    private static func errorMessage(from object: Any) -> String? {
        if let message = ComicRuleAPIResolver.stringValue(object) {
            return message
        }
        ...
        let message = ComicRuleAPIResolver.stringValue(dictionary["message"])
        let code = ComicRuleAPIResolver.stringValue(dictionary["code"])
            ?? ((dictionary["extensions"] as? [String: Any]).flatMap { extensions in
                return ComicRuleAPIResolver.stringValue(extensions["code"])
            })
        ...
        if let extensions = dictionary["extensions"] as? [String: Any] {
            for key in ["current", "limit", "requested"] {
                if let value = ComicRuleAPIResolver.stringValue(extensions[key]) {
