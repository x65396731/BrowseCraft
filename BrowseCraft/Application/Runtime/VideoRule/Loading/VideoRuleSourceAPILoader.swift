...
struct VideoRuleSourceAPILoader {
    ...
    func loadList(
        ...
        let resolution: VideoRuleJSONArrayResolution = VideoRuleAPIResolver.arrayResolution(
            at: api.itemPath,
            in: loaded.object
        )
        ...
        let sortedObjects: [Any] = VideoRuleAPIResolver.sorted(
            resolution.values.enumerated().map { offset, value in
                return (
                    offset: offset,
                    value: value,
                    order: api.orderPath.flatMap { path in
                        return VideoRuleAPIResolver.doubleValue(
                            VideoRuleAPIResolver.firstJSONValue(at: path, in: value)
                        )
                    }
                )
            },
        ...
        for itemObject: Any in sortedObjects {
            guard let title: String = Self.nonEmpty(
                VideoRuleAPIResolver.stringValue(
                    VideoRuleAPIResolver.firstJSONValue(
            ...
            seenDetailURLs.insert(VideoRuleAPIResolver.canonicalURLKey(detailURL)).inserted else {
                continue
            }
            let idCode: String? = api.fields.idCodePath.flatMap { path in
                return Self.nonEmpty(
                    VideoRuleAPIResolver.stringValue(
                        VideoRuleAPIResolver.firstJSONValue(at: path, in: itemObject)
                    )
                )
            }
    ...
    func loadDetail(
        ...
        let resolution: VideoRuleJSONObjectResolution = VideoRuleAPIResolver.objectResolution(
            at: api.itemPath,
            in: loaded.object
        )
        ...
        let idCode: String? = api.fields.idCodePath.flatMap { path in
            return Self.nonEmpty(
                VideoRuleAPIResolver.stringValue(
                    VideoRuleAPIResolver.firstJSONValue(at: path, in: detailObject)
                )
            )
        }
        ...
        let attributes: [VideoRuleParsedDetailAttribute] = (api.fields.metadata ?? []).compactMap { field in
            guard let value: String = Self.nonEmpty(
                VideoRuleAPIResolver.stringValue(
                    VideoRuleAPIResolver.firstJSONValue(at: field.valuePath, in: detailObject)
                )
            ) else {
                return nil
            }
    ...
    func loadEpisodes(
        ...
        for groupObject: Any in groupObjects {
            let itemResolution: VideoRuleJSONArrayResolution = VideoRuleAPIResolver.arrayResolution(
                at: api.itemPath,
                in: groupObject
            )
            try self.validateArrayState(
            ...
            let groupIDCode: String? = api.groupFields?.idCodePath.flatMap { path in
                return Self.nonEmpty(
                    VideoRuleAPIResolver.stringValue(
                        VideoRuleAPIResolver.firstJSONValue(at: path, in: groupObject)
                    )
                )
            }
            ...
            for itemObject: Any in sortedObjects {
                ...
                seenPlayURLs.insert(VideoRuleAPIResolver.canonicalURLKey(playURL)).inserted else {
                    continue
                }
                let idCode: String? = api.fields.idCodePath.flatMap { path in
                    return Self.nonEmpty(
    ...
    private func loadJSON(
        ...
        do {
            apiURLString = try VideoRuleAPIResolver.resolveTemplate(
                apiURLTemplate,
            ...
            request = try VideoRuleAPIResolver.resolvedRequest(
                mergedRequest,
                context: templateContext
            )
        } catch {
        ...
        switch VideoRuleAPIResolver.evaluateResponse(json: jsonObject, policy: responsePolicy) {
        case .allowParsing:
            break
    ...
    private func listItemURL(
        ...
        if let template: String = api.fields.detailURLTemplate {
            ...
            rawValue = api.fields.detailURLPath.flatMap { path in
                return VideoRuleAPIResolver.stringValue(
                    VideoRuleAPIResolver.firstJSONValue(at: path, in: currentJSON)
                )
            }
        }
        return VideoRuleAPIResolver.absoluteHTTPURL(rawValue, relativeTo: baseURL)
    }
    ...
    private func scalarMatch(
        ...
        guard let path: String,
              let scalar: APIResponseScalar = VideoRuleAPIResolver.responseScalar(
                VideoRuleAPIResolver.firstJSONValue(at: path, in: object)
              ) else {
            return nil
        }
    ...
    private func resolveOutputTemplate(
        ...
        do {
            return try VideoRuleAPIResolver.resolveTemplate(
                template,
